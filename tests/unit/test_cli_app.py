...
class CliSmokeTests(unittest.TestCase):
    def test_version_never_connects_to_gmes(self):
        with patch.object(app.application, "sign_in_ok") as sign_in:
    ...
    def test_run_builds_one_typed_spec_per_screen(self):
        with patch.object(app.application, "sign_in_ok", return_value=True), \
             patch.object(app.application, "connect_gmes", return_value=type("W", (), {"close": lambda s: None})()), \
             patch.object(app.application, "run_many", return_value=[] ) as run_many:
    ...
    def test_data_read_passes_the_named_limit_to_the_reader(self):
        ...
        with patch.object(app.application, "sign_in_ok", return_value=True), \
             patch.object(app.application, "connect_gmes", return_value=type("W", (), {"close": lambda s: None})()), \
             patch.object(app.application, "read_dataset", return_value=response) as read:
