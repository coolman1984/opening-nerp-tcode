...
> | 4 | Restore the legacy workflow tests; add a guard test proving the legacy entrance loads **zero** `gmes.*` modules; `tests/test_gmes_core.py` and all N-ERP tests stay green | **next** — starts by repairing `tests/unit/test_gmes_workflow.py`, which step 3 knowingly broke: it asserts the bridge (`gmes_main(["workflow"])`) that no longer exists |
