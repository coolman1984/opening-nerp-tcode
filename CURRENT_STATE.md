...
> | **E1** | **Port `--disable-popup-blocking` into `cdp_common.py`** — legacy cannot open the AD SSO popup without it; live-proven in 56.1. N-ERP suite gates it (shared file) | **not started — do before any deletion** |
> | **E2** | **Port the screenshot-target fix** to the G-MES path; do NOT change `get_page_tab`'s `"nerps"` default | **not started — do before any deletion** |
