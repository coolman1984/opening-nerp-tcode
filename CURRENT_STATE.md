...
| 6b | Unified CLI — initial command surface | in progress — `python -m gmes` and the package entry point dispatch offline `version`, typed `login`, sequential `run`, and `data forms/read`; `data read` fixes the old limit drift with `--limit`; remaining command families and workflow are pending |
