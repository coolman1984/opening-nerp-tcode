...
| 5c | `discovery/{screen_discovery,screen,catalogue,fingerprint}.py` | not started |
| 5d | Dataset paging fix in `query/dataset_reader.py` (`read_dataset_paged`) | not started — `query/` currently holds the unchanged, single-shot-read faithful port only |
| 5e | `export/{excel,csv_export,naming}.py` | not started |
