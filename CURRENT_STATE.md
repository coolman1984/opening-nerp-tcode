...
| 6b | Unified CLI — initial command surface | in progress — `python -m gmes` and the package entry point now dispatch offline `version`, typed `login`, and validated sequential `run`; remaining command families and workflow are still pending |
