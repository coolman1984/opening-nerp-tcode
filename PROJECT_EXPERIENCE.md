...
| **Q3442UM00** Quality Set Tracking (menu `MQM0116`) | NOT recorded: a per-unit lookup (`*CN/SN/IMEI/ASSY/UN`), not a division/date report - the standing recipe does not apply. || **P1112UM00** Production Plan by Order(Line) | Quick Views `P1112WM00`/`WM01` are other menus, not recorded. Verified by `planYmd`. The 16 Sep recording refused to replay ("shape changed") and was re-recorded with `--relearn` (84.22). |
